...
LABEL_COL = "event"
PREDICTOR_COLS = ["Rscore", "MWT", "LAs", "LVGRS", "LVGLS"]
...
def validate_data(df):
    ...
    if y.isna().any() or not y.isin([0, 1]).all() or set(y.unique()) != {0, 1}:
...
def specificity_at_target_sensitivity_from_prob(y_true, y_prob, target=0.95):
    fpr, tpr, _ = roc_curve(
        y_true, y_prob, drop_intermediate=False
    )
    eligible = tpr >= target
...
def run_outer_fold(
    ...
    split_seed = SEED + repeat_id * 1000 + fold_id * 100
    fold_seed = split_seed + MODEL_ORDER.index(model_name)
    set_seed(fold_seed)
...
# Summaries and bootstrap confidence intervals
...
def make_ranking(summary):
    ...
    wide["Meets sensitivity target"] = (
        wide["Sensitivity"] >= TARGET_SENSITIVITY
    )
    eligible = wide[wide["Meets sensitivity target"]].sort_values(
        ["Specificity", "NPV", "AUC", "Brier Score"],
        ascending=[False, False, False, True]
    ).copy()
    eligible.insert(0, "Rank", np.arange(1, len(eligible) + 1))

    ineligible = wide[~wide["Meets sensitivity target"]].sort_values(
        ["Sensitivity", "Specificity", "AUC"],
        ascending=[False, False, False]
    ).copy()
    ineligible.insert(0, "Rank", np.nan)

    ranking = pd.concat([eligible, ineligible], ignore_index=True)
    ranking["Selection rule"] = (
        "Eligible if locked-threshold nested-CV sensitivity >=0.95; "
        "eligible models ranked by specificity, NPV, AUC, and Brier score"
    )
    return ranking
...
def save_outputs(
    ...
    print("\nModel ranking:")
    print(ranking[[
        "Rank", "Model", "Meets sensitivity target",
        "Sensitivity", "Specificity", "NPV", "AUC", "Brier Score"
